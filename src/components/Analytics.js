...
const Analytics = ({ alltransaction }) => {
  //catagory data
  const catagories = ['salary','tip','project','food','movie','travel','shopping','bills','medical','fee','tax'];
  // Calculate total transactions and percentages
  ...
  return (
    <>
      ...
      <div className='row mt-3'>
        <div className='col-md-4'>
          <h4>Catagorywise Income</h4>
          {catagories.map((catagory) => {
              const amount = alltransaction
              .filter((transaction) => transaction.type === 'income' && transaction.catagory === catagory)
        ...
        <div className='col-md-4'>
          <h4>Catagorywise Expense</h4>
          {catagories.map((catagory) => {
              const amount = alltransaction
              .filter((transaction) => transaction.type === 'expense' && transaction.catagory === catagory)
              .reduce((acc, transaction) => acc + transaction.amount, 0);
              return (
                amount > 0 && (
                <div className='card'>
